...
function generateDayHours() {
	...
	for (var i = 0; i < Object.keys(obj).length; ++i) {
		...
		if (obj[i]["conso"]>0) {
			console.log('DELETE FROM \'Meter\' WHERE devicerowid='+devicerowid+' and date = \''+req_date+'\'; INSERT INTO \'Meter\' (DeviceRowID,Usage,Value,Date) VALUES ('+devicerowid+', \''+Math.round(obj[i]["conso"]*100)+'\', \''+Math.round(cumul*1000)/1000+'\', \''+req_date+'\');') ;
